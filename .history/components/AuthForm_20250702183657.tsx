          ...
          {currentFields.map((field, index) => {
            ...
            return (
              <motion.div key={field} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                <button
                  onClick={() => {
                    if (field === "artgang") router.push("/artgang");
                    else if (field === "account_created" && !user) setShowAuthModal(true);
                  ...
                  style={{
                    background: editingFieldIndex === index ? "#0af" : isComplete ? "#1a1a1a" : "#1a0000",
                    color: isComplete ? "#39ff14" : "#f87171",
                    padding: "10px 16px",
                    borderRadius: 10,
                    border: "none",
                    minWidth: 160,
                  ...
                  {field === "artgang" ? "artgang →" : `${field}: ${isComplete ? userData[field] : "(edit)"}`}
                </button>
              </motion.div>
            );
          })}
        ...
        <AnimatePresence>
          {showAuthModal && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.7)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center" }}
