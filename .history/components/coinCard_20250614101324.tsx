...
function LazyRender({ children, style }: { children: React.ReactNode, style: React.CSSProperties }) {
  ...
  return <div ref={ref} style={{ ...style, opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(40px)', transition: 'all 0.6s ease-out' }}>{inView ? children : null}</div>
}
...
function CoinCard({
  ...
  return (
    ...
    onMouseLeave={(e) => (e.currentTarget.style.transform = 'scale(1)')}>
      ...
      <button onClick={(e) => { e.stopPropagation(); showInput ? handleBuy(coin) : setShowInput(true) }}
