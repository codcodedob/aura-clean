...
// Two onboarding flows for business and arc session

export function BusinessOnboardingFlow() {
  const steps = ["Create Account", "Complete Profile", "Choose Art Role", "Upload Portfolio", "Set Up Wallet", "Get Verified", "Go Public"];
  return (
    <MotionSection>
      {steps.map(step => (
        <MotionCard key={step} title={step}>Business flow step: {step}</MotionCard>
      ))}
    </MotionSection>
  );
}

export function ArcSessionOnboardingFlow() {
  const steps = [
    "Halo ID",
    "Birthday",
    "Age",
  ...
  return (
    <MotionSection>
      {steps.map(step => (
        <MotionCard key={step} title={step}>Arc session step: {step}</MotionCard>
      ))}
    </MotionSection>
  );
}
