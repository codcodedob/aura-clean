...
export default function Home() {
  ...
  // 1) Auth: fetch initial user & subscribe to changes
  useEffect(() => {
  ...
  // 2) Load coins whenever user changes
  useEffect(() => {
    if (!user) {
      setCoins([])
      return
  ...
  // 3) Overlay AuthForm if not signed in
  if (!user) {
    return (
      <div style={{ background: '#000', minHeight: '100vh' }}>
        <div style={overlayStyle}>
          <AuthForm onAuth={() => { /* supabase listener picks it up */ }} />
        </div>
      </div>
    )
  }
...
// Force SSR so auth runs per request
