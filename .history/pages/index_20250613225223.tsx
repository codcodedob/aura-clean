...
function CoinCard({ coin, investmentAmounts, setInvestmentAmounts, handleBuy, isSponsored }: { coin: Coin; investmentAmounts: { [key: string]: number }; setInvestmentAmounts: React.Dispatch<React.SetStateAction<{ [key: string]: number }>>; handleBuy: (coin: Coin) => void; isSponsored?: boolean }) {
  return (
    <div style={{ background: '#111', padding: '1rem', borderRadius: 8, border: '1px solid #444', position: 'relative' }}>
      ...
      />
      <button onClick={() => handleBuy(coin)} style={{ marginTop: 10, padding: '6px 16px', width: '100%' }}>
        Buy
      </button>
