...
export default function Business() {
  ...
  const handleSubmitModal = async (e: React.FormEvent) => {
    ...
    try {
      // Create or fetch coin for user
      const coin = await getOrCreateUserCoin(user.id, {
        name: formState.coinName || user.email.split("@")[0] + "'s Coin",
        symbol: (formState.coinName || "COIN").slice(0, 6).toUpperCase(),
        scope: [formState.coinScope].filter(Boolean),
        dividends_eligible: !!formState.coinDividends,
      });

      if (modalInfo?.key === "artist-coin") {
        // Artist Coin: mark as non-active by default
        ...
        alert("Artist Coin and onboarding contract created! You can activate your coin from your profile.");
        setModalInfo(null);
      } else if (modalInfo) {
        // Paid flow: Save activity then trigger Stripe checkout
        await supabase.from("activities").insert({
          user_id: user.id,
          type: modalInfo.key,
          details: JSON.stringify({ ...formState, coinId: coin.id }),
          status: "pending",
          created_at: new Date().toISOString(),
        });
        // Stripe
