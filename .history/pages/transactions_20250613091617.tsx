...
export default function TransactionsPage() {
  ...
  return (
    ...
    <div style={{ marginBottom: '1rem' }}>
      <a href="/" style={{ color: '#0af', textDecoration: 'underline' }}>← Back to Home</a>
    </div>
    ...
    <div style={{ marginTop: '3rem' }}>
        <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>My Coin Holdings</h2>
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '1rem' }}>
          <thead>
            <tr>
              <th style={{ borderBottom: '1px solid #555', padding: '0.5rem', textAlign: 'left' }}>Coin</th>
              <th style={{ borderBottom: '1px solid #555', padding: '0.5rem', textAlign: 'right' }}>% Owned</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map(tx => (
              <tr key={tx.id + '-owned'}>
                <td style={{ padding: '0.5rem' }}>{tx.coin_name}</td>
                <td style={{ padding: '0.5rem', textAlign: 'right' }}>{((tx.amount / tx.projected_cap) * 100).toFixed(2)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
