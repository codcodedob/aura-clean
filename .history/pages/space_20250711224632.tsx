...
// Modal shown when user is not logged in
...
// Modal shown when deleting account
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: "0 4px 20px rgba(0,0,0,0.4)", textAlign: "center"
      }}>
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 12, justifyContent: "center" }}>
          ...
          <button onClick={onConfirm} style={{ background: "#ef4444", color: "#fff", padding: "8px 16px" }}>
            Delete Account
          </button>
        </div>
      </div>
...
export default function Space() {
  ...
  async function handleDeleteConfirmed() {
    try {
      // This requires Admin API permission — otherwise, redirect to support
      const { error } = await supabase.auth.admin.deleteUser(user!.id);
      if (error) {
        console.error("Failed to delete account:", error.message);
        alert("Unable to delete account. Contact support.");
      } else {
  ...
  return (
    <div style={{ padding: 24, background: "#f9fafb", minHeight: "100vh" }}>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        {["Art", "Entertainment", "Cuisine", "Fashion", "Health", "Science"].map((dept) => (
          <div key={dept} style={{ margin: "8px 0" }}>
            <span>{dept}</span>
            <div style={{
              height: 10, background: "#e5e7eb", borderRadius: 6, marginTop: 2, width: "100%", maxWidth: 400
            }}>
              <div style={{
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <div style={{
          background: "#fff", borderRadius: 12, padding: 20, maxHeight: 300, overflowY: "auto"
        }}>
          {user ? (
            activities.length ? (
      ...
      <section>
        ...
        <div style={{
          background: "#fff", borderRadius: 12, padding: 20, maxWidth: 400
        }}>
          {user ? (
            <>
