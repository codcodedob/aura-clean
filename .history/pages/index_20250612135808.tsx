...
export default function Home() {
  ...
  const cityCoins = visibleCoins.filter(c => c.name.startsWith('City'))
  const userCoins = visibleCoins.filter(c => !c.name.startsWith('City') && c.user_id !== user?.id)
  ...
  const createCity1Coin = async () => {
    ...
    const { error } = await fetch('/api/create-city-coin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ adminEmail: user.email })
    }).then(res => res.json())
    alert(error ? `❌ Error creating City ${nextNum}` : `✅ City ${nextNum} created`)
  ...
  const [editBuffer, setEditBuffer] = useState<Record<string, Partial<Coin>>>({})

  const handleAdminFieldChange = (coinId: string, field: 'tagline' | 'vision', value: string) => {
    setEditBuffer(prev => ({
      ...prev,
      [coinId]: {
        ...prev[coinId],
  ...
  const saveCoinEdits = async (coinId: string) => {
    const updates = editBuffer[coinId]
    if (!updates) return
    ...
    if (error) {
      ...
      alert('✅ Coin updated!')
      setCoins(prev =>
        prev.map(c => (c.id === coinId ? { ...c, ...updates } : c))
      )
      setEditBuffer(prev => {
  ...
  return (
    <div style={{ display: 'flex', background: '#000', color: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ flex: 1, padding: '1rem', borderRight: '1px solid #333' }}>
        <h2 style={{ textAlign: 'center' }}>City level investment options</h2>
        {cityCoins.map(coin => renderCoinCard(coin, '#a22'))}
      </div>
      ...
      <div style={{ flex: 1, padding: '1rem', borderLeft: '1px solid #333' }}>
        <h2 style={{ textAlign: 'center' }}>Active digital region</h2>
        {userCoins.map(coin => renderCoinCard(coin, '#3a5'))}
      </div>
    </div>
  )
}
