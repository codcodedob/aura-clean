...
export default function Home() {
  ...
  useEffect(() => {
    if (!user) return
    const mine = Array.isArray(coins) ? coins.find(c => c.user_id === user.id) || null : null
    setMyCoin(mine)
  }, [coins, user])
  ...
  <span style={{
    ...
    {c.rarity ?? 'common'}
            ...
            <button
              disabled={buyingId === c.id || c.user_id === user?.id}
              onClick={() => handleStripeBuy(c)}
              style={{
                background: c.user_id === user?.id ? '#555' : '#1e1',
                color: '#fff',
                border: 'none',
                borderRadius: 6,
                padding: '6px 12px',
                cursor: c.user_id === user?.id ? 'not-allowed' : 'pointer',
              }}
            >
              {c.user_id === user?.id ? 'Your Coin' : (buyingId === c.id ? 'Processing…' : 'Buy')}
            </button>
          </div>
        ))}
      </div>
