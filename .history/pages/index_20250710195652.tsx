...
export default function Home() {
  ...
  // Fetch user on mount and fallback to auto-login admin demo user
  useEffect(() => {
    async function fetchUser() {
      ...
      if (error || !data.user) {
        ...
        if (anonError && anonError.message.includes("Invalid login credentials")) {
          await supabase.auth.signUp({ email: ADMIN_EMAIL, password: "testpassword" });
  ...
  const handleSignUp = async () => {
    ...
    setAuthMessage(error ? error.message : "Check your email for confirmation link.");
    setAuthLoading(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 12 }}>Your Aura Coins</h2>
        {!user ? (
          <AuthPanel
            email={email}
            setEmail={setEmail}
          ...
          <>
            ...
            {coins.length === 0 ? (
              <p style={{ color: "#999" }}>No coins found.</p>
            ) : (
              coins.map((coin) => (
                <motion.div
                  key={coin.id}
                  ...
                  onClick={() => console.log("Clicked coin:", coin.name)}
                >
...
// AuthPanel: login/signup form props
function AuthPanel({
  email,
  setEmail,
  password,
