...
export default function Home() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'url(/orchestra-bg.jpg) center/cover no-repeat fixed', color: '#fff', overflow: 'hidden' }}>
      <div style={{ flex: 1, padding: 20, borderRight: '1px solid #333', backgroundColor: 'rgba(0,0,0,0.85)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 20, borderLeft: '1px solid #333', borderRight: '1px solid #333', overflowY: 'auto', maxHeight: '100vh', backgroundColor: 'rgba(0,0,0,0.85)' }}>
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search coins" style={{ width: '100%', padding: 8, marginBottom: 20, backgroundColor: '#000', color: '#fff', border: '1px solid #444', borderRadius: 6 }} />
        <h1>AURA</h1>
        <img src="aura-avatar.png" id="helmet-hud" alt="Aura Helmet HUD" onClick={() => {
          ...
          recognition.start();
        }} style={{ width: '100%', maxWidth: 300, objectFit: 'contain', margin: '20px 0', cursor: 'pointer', borderRadius: 12, filter: 'drop-shadow(0 0 8px #0f0)' }} />
        <AvatarClothingSelector />
        <p style={{ marginTop: 12 }}>{user?.email || 'Not signed in'}</p>
        <button onClick={async () => { await supabase.auth.signOut(); window.location.replace('/login'); }} style={{ marginTop: 8, padding: '6px 12px', background: '#222', color: '#fff', border: '1px solid #444', borderRadius: 6 }}>Logout</button>
        {userCoin ? (
          <CoinCard coin={userCoin} investmentAmounts={investmentAmounts} setInvestmentAmounts={setInvestmentAmounts} handleBuy={handleStripeBuy} />
        ) : (
          <p>You don't have a coin yet.</p>
        )}
        <Link href="/transactions" style={{ color: '#0af', marginTop: 24 }}>View My Transactions</Link>
      </div>

      <div style={{ flex: 1, padding: 20, borderLeft: '1px solid #333', overflowY: 'auto', maxHeight: '100vh', backgroundColor: 'rgba(0,0,0,0.85)' }}>
        ...
        {[' Art', ' Entertainment', ' Cuisine', ' Fashion', ' Health & Fitness', 'Science & Tech', ' Community Clipboard'].map((dept, i) => (
          <div key={i} style={{ background: '#000', marginBottom: 16, padding: 16, borderRadius: 12, border: '1px solid #444' }}>
...
function CoinCard({ coin, investmentAmounts, setInvestmentAmounts, handleBuy }: { coin: Coin, investmentAmounts: { [key: string]: number }, setInvestmentAmounts: React.Dispatch<React.SetStateAction<{ [key: string]: number }>>, handleBuy: (coin: Coin) => void }) {
  const [showInput, setShowInput] = useState(false)
  return (
    <div style={{ background: '#000', margin: '1rem 0', padding: '1rem', borderRadius: 8, border: '1px solid #444', textAlign: 'center' }}>
      <strong style={{ fontSize: 18 }}>{coin.emoji ?? '🪙'} {coin.name}</strong>
      <p>${coin.price.toFixed(2)} · cap {coin.cap}</p>
      {showInput && (
        <input type="number" placeholder="Enter amount" min={coin.price} value={investmentAmounts[coin.id] ?? ''} onChange={e => setInvestmentAmounts(prev => ({ ...prev, [coin.id]: parseFloat(e.target.value) }))} style={{ width: '80%', padding: 6, marginTop: 8 }} />
      )}
      <button onClick={() => { setShowInput(true); if (showInput) handleBuy(coin) }} style={{ marginTop: 10, padding: '6px 16px' }}>
        {showInput ? 'Confirm' : 'Buy'}
      </button>
