...
export default function Space() {
  ...
  // Fetch Enteractive pyramid data
  useEffect(() => {
    supabase
      .from("enteractive")
      ...
      .then(({ data, error }) => {
        if (error) {
          ...
          // Ensure project_scope is numeric and link is string
          const filtered = data
            .filter(
              (item): item is EnteractiveItem =>
                !!item.id && !!item.name && typeof item.project_scope === "number" && typeof item.link === "string"
            );
          setEnteractiveData(filtered);
        }
      });
  }, []);
  ...
  return (
    <div style={{ padding: 24, background: "#f9fafb", minHeight: "100vh" }}>
      {/* Enteractive Pyramid at top */}
      <div style={{ marginBottom: 32, width: "100%", height: 400 }}>
        <EnteractivePyramid data={enteractiveData} width={window.innerWidth - 48} height={400} />
      </div>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        >
          ...
          <button onClick={() => requireAuth(() => alert("Open health data panel"))}>Connect Health Data</button>
        </div>
      </section>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        >
          ...
          <button onClick={() => requireAuth(() => router.push("/inbox"))}>Go to Messaging & Groups</button>
        </div>
      </section>
            ...
            activities.length ? (
              activities.slice(0, 8).map((a) => (
                <div key={a.id} style={{ borderBottom: "1px solid #e5e7eb", padding: "12px 0" }}>
                  <div>
                    <b>Type:</b> {a.type} &nbsp; <b>Status:</b> {a.status} &nbsp; <b>State:</b> {a.state}
                  </div>
                  <div style={{ fontSize: 14, color: "#666" }}>{a.detail || "No detail"}</div>
                  <div style={{ fontSize: 12, color: "#999" }}>
                    {a.activityStartTimestamp ? new Date(a.activityStartTimestamp).toLocaleString() : ""}
                  </div>
                </div>
              ))
            ) : (
