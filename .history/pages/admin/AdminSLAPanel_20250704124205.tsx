...
// Include the primary key in the row type
type SLARow = SLAData & { id: string }

export default function AdminSLAPanel() {
  // Use typed state instead of any
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase
        .from<SLARow, SLARow>('sla_compliance')
        .select('*')
        .eq('id', recordId)
        .single()
