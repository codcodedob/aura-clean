...
export default function AdminSiteVisitsPanel() {
  ...
  // Fetch visits with filters and sorting
  ...
  return (
    <div style={{
      ...
      <h3 style={{ color: '#0af', fontWeight: '700', marginBottom: 12 }}>Recent Site Visits</h3>
      ...
      <div style={{ marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <label>
          Order: 
          <select value={sortOrder} onChange={e => setSortOrder(e.target.value as any)} style={{ marginLeft: 8 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', color: '#fff' }}>
          ...
          <tbody>
            {visits.length === 0 ? (
              ...
              visits.map((visit) => (
                <tr key={visit.id} style={{ borderBottom: '1px solid #2e3a4e' }}>
                  <td style={{ padding: 8 }}>{new Date(visit.timestamp).toLocaleString()}</td>
                  <td style={{ padding: 8 }}>{visit.url}</td>
                  <td style={{ padding: 8 }}>{visit.ip}</td>
                  <td style={{ padding: 8, fontSize: 12 }}>{visit.user_agent}</td>
