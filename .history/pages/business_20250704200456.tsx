...
export default function Business() {
  ...
  // Utility to get or create a coin for user
  const getOrCreateUserCoin = async (user: any) => {
  ...
  const handleSubmitModal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalInfo?.key === "artist-coin") {
        ...
        // Paid flows: ensure user has a coin to use for order/ownership
        const coinId = await getOrCreateUserCoin(user);

        // Save the order activity as before
        await supabase.from("activities").insert({
          user_id: user.id,
          type: modalInfo.key,
        ...
        // Stripe checkout: MUST include coinId
