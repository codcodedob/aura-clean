...
// Dynamically import EnteractivePyramid to avoid SSR issues with Three.js etc
...
export default function Space() {
  ...
  const [enteractiveData, setEnteractiveData] = useState<
    { id: string; name: string; project_scope: number; link: string }[]
  >([]);
  const [loadingPyramid, setLoadingPyramid] = useState(true);
  ...
  // Fetch enteractive data once on mount
  useEffect(() => {
    supabase
      .from("enteractive")
  ...
  // Generate onboarding progress once on client
  useEffect(() => {
    setRandomProgress(
  ...
  return (
    <div style={{ padding: 24, background: "#f9fafb", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: 32, display: "flex", justifyContent: "center" }}>
        {!loadingPyramid && enteractiveData.length > 0 ? (
          <EnteractivePyramid
            data={enteractiveData}
            width={350}
            height={350}
          />
        ) : (
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        {["Art", "Entertainment", "Cuisine", "Fashion", "Health", "Science"].map((dept, i) => (
          <div key={dept} style={{ margin: "8px 0" }}>
            ...
            >
              <div
                style={{
                  width: `${randomProgress[i]?.toFixed(0) ?? 0}%`,
                  background: "#3b82f6",
                  height: "100%",
                  borderRadius: 6,
