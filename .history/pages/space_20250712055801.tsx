...
// Dynamically import EnteractivePyramid with no SSR (because it uses canvas/three.js)
...
export default function Space() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Fetch enteractive data for pyramid
      supabase
        .from("enteractive")
        ...
        .then(({ data, error }) => {
          if (error) {
            ...
            // Sanitize and cast project_scope to number just in case
  ...
  // Executive check (same as before)
  useEffect(() => {
    if (!user) return;
