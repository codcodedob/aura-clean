...
export default function Home() {
  ...
  // Fetch user on mount & handle fallback admin login/signup
  useEffect(() => {
    async function fetchUser() {
      const { data, error } = await supabase.auth.getUser();
      if (error || !data.user) {
        // Auto-login/signup fallback for demo admin
  ...
  // Fetch AuraCoins with safe conversion & error handling
  useEffect(() => {
  ...
  // Fetch department media sorted by slot ascending
  useEffect(() => {
  ...
  return (
    ...
    >
        ...
        {coins.length === 0 ? (
          ...
          coins.map((coin) => (
            <motion.div
              ...
              onClick={() => {
                // Placeholder: show coin details or initiate purchase flow
      ...
      {/* Center Panel: Immersive View / Avatar Closet */}
      <div
        style={{
          flex: 2,
          background: "#161b22",
...
// Countdown component for right panel
