...
export default function TransactionsPage() {
  ...
  const exportToCSV = () => {
    const header = ['Coin', 'Amount', 'Date']
    const rows = transactions.map(tx => [tx.coin_name, tx.amount.toFixed(2), new Date(tx.created_at).toLocaleString()])
  ...
  return (
    <div style={{ padding: '2rem', backgroundColor: '#000', color: '#fff', minHeight: '100vh' }}>
      ...
      <p style={{ fontSize: '1.2rem', marginBottom: '1rem' }}>Total Invested: ${totalInvested.toFixed(2)}</p>
