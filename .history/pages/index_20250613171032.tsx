...
export default function Home() {
  ...
  <AutoSizer>
    {({ height, width }) => (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 20, borderLeft: '1px solid #333', borderRight: '1px solid #333', overflowY: 'auto', maxHeight: '100vh' }}>
        ...
        <h1>AURA</h1>
        <img
          src="/15588913-d19d-45c3-ba7a-78c8c4cbe88f.png" id="helmet-hud"
          alt="Aura Helmet HUD"
          onClick={() => {
  ...
  {['🎨 Art', '🎬 Entertainment', '🍽️ Cuisine', '👗 Fashion', '💪 Health & Fitness', '🔬 Science & Tech', '📋 Community Clipboard'].map((dept, i) => (
    <div key={i} style={{ background: 'linear-gradient(135deg, #111, #1c1c1c)', marginBottom: 16, padding: 16, borderRadius: 12, border: '1px solid #0af', boxShadow: '0 0 12px rgba(0, 170, 255, 0.3)' }}>
      ...
      <button style={{ marginTop: 10, padding: '10px 20px', background: '#0af', color: '#000', borderRadius: 8, fontWeight: 'bold', border: 'none', boxShadow: '0 0 6px #0af' }}>Open</button>
    </div>
  ))}
</div>
    </div>
