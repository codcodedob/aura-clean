  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#000', color: '#fff' }}>
      <div style={{ flex: 1, padding: 20, borderRight: '1px solid #333' }}>
        ...
        {othersCoins.slice(0, Math.ceil(othersCoins.length / 2)).map(coin => (
      ...
      <div style={{ flex: 1, padding: 20, borderLeft: '1px solid #333', borderRight: '1px solid #333' }}>
        ...
        />
        {userCoin ? (
          <CoinCard coin={userCoin} investmentAmounts={investmentAmounts} setInvestmentAmounts={setInvestmentAmounts} />
        ) : (
          <p style={{ textAlign: 'center' }}>You don’t have an AuraCoin yet.</p>
        )}
      </div>

      <div style={{ flex: 1, padding: 20, borderLeft: '1px solid #333' }}>
        <h2 style={{ textAlign: 'center' }}>+1000</h2>
        {othersCoins.slice(Math.ceil(othersCoins.length / 2)).map(coin => (
          <CoinCard key={coin.id} coin={coin} investmentAmounts={investmentAmounts} setInvestmentAmounts={setInvestmentAmounts} />
        ))}
      </div>
    </div>
...
function CoinCard({ coin, investmentAmounts, setInvestmentAmounts }: {
  ...
  const handleBuy = async () => {
    ...
    if (!amount || amount < coin.price) {
      alert(`Minimum investment is $${coin.price.toFixed(2)}`)
      return
    }
    try {
  ...
  return (
    <div style={{
      ...
      />
      <button onClick={handleBuy} style={{ marginTop: 10, padding: '6px 16px' }}>Buy</button>
    </div>
  )
}
