...
export default function Business() {
  ...
  const handleFormChange = (
    ...
    if (
      type === "checkbox" &&
      e.target instanceof HTMLInputElement // type guard!
    ) {
      setFormState((prev) => ({
        ...prev,
  ...
  const handleSubmitModal = async (e: React.FormEvent) => {
    ...
    try {
      // Always get or create coin for user (free or paid)
      const coinId = await getOrCreateUserCoin(user);

      if (modalInfo?.key === "artist-coin") {
        // Update user's coin with entered data
        ...
        await supabase.from("activities").insert({
          ...
          type: "onboarding",
          action: "Created Coin",
          details: JSON.stringify({ coin_id: coinId }),
          status: "present",
        ...
        alert("Your coin is created! (set as inactive until activated)");
        setModalInfo(null);
      } else if (modalInfo) {
        // Save order activity
        await supabase.from("activities").insert({
          user_id: user.id,
          type: modalInfo.key,
          details: JSON.stringify({ ...formState, coin_id: coinId }),
          status: "pending",
          created_at: new Date().toISOString(),
        });
        ...
        const res = await fetch("/api/create-checkout", {
          ...
          body: JSON.stringify({
            ...
            userId: user.id,
            coinId, // now required by your API!
            ...formState
          }),
        });
