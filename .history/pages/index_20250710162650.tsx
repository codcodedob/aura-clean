...
const FocusedAvatar = lazy(() => import("@/components/FocusedAvatar")) as React.LazyExoticComponent<
  React.ComponentType<{}>
>;
const FullBodyAvatar = lazy(
  () => import("@/components/FullBodyAvatar")
) as React.LazyExoticComponent<React.ComponentType<{ modelPaths: string[] }>>;
...
export default function Home() {
  ...
  // NEW: Signup handler integrated with your create-user API
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!data.user) {
      setSignupError("Signup failed: no user returned");
      return;
    }

    // 2) Call your create-user API to create Stripe customer and insert user in DB
    ...
    if (!res.ok) {
      ...
      setSignupError(json?.error || "Failed to create Stripe customer");
      return;
    }
          ...
          <div style={{ height: "100%" }}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <button
                ...
                style={{ marginRight: 8, color: filter === "stock" ? "#0af" : "#ccc" }}
              >
                Stocks
              </button>
          ...
          {!user ? (
            ...
            >
              ...
              <form onSubmit={handleSignup}>
                ...
                <button
                  type="submit"
                  style={{ background: "#0af", color: "#000", padding: "10px 16px", borderRadius: 6, fontWeight: "bold", width: "100%" }}
                >
                ...
                <button
                  ...
                  style={{ background: "transparent", color: "#0af", marginTop: 10, width: "100%", border: "none", cursor: "pointer" }}
                >
                  {signupMode ? "Have an account? Log In" : "New user? Create account"}
                </button>
              </form>
            ...
            <div style={{ marginTop: 24 }}>
              <p style={{ color: "#0af" }}>Logged in as {user.email}</p>
              <button
        ...
        >
          ...
          {user?.email === ADMIN_EMAIL && (
            <>
              ...
              {message && <p style={{ marginTop: 12, color: "#0af" }}>{message}</p>}
            </>
          )}
        </div>
      )}
