...
function PyramidSlice({
  ...
  // Create text geometry memoized
  const textGeometry = useMemo(() => {
    if (!font) return null;
    return new TextGeometry(label, {
      font,
      size: 0.15,
      height: 0.05,
      curveSegments: 12,
  ...
  // Positioning and slice size
  const angle = (index / totalSlices) * Math.PI * 2;
  const radius = 2;
  const sliceHeight = (projectScope / 100) * 1;
  ...
  return (
    ...
    >
      ...
      {textGeometry && (
        <mesh geometry={textGeometry} position={[-0.4, sliceHeight + 0.15, 0]}>
...
export default function Space() {
  ...
  const [enteractives, setEnteractives] = useState<
  ...
  // Fetch enteractive data on mount
  useEffect(() => {
    supabase
      .from("enteractive")
