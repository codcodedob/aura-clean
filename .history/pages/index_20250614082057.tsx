...
export default function Home() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'url(/orchestra-bg.jpg) center/cover no-repeat', color: '#fff', overflow: 'hidden' }}>
      <div style={{ flex: 1, padding: 20, borderRight: '1px solid #333', overflow: 'auto' }}>
          ...
          <AutoSizer>
            {({ height, width }) => (
              <List height={height} itemCount={othersCoins.length} itemSize={210} width={width}>
                {({ index, style }) => (
                  <LazyRender style={style} key={othersCoins[index].id}>
                    <CoinCard
      ...
      <div style={{ flex: 1.1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 20, borderLeft: '1px solid #333', borderRight: '1px solid #333', overflowY: 'auto', maxHeight: '100vh' }}>
        <input
          type="text"
          value={search}
