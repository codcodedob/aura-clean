...
export default function Home() {
  ...
  const renderAvatar = () => {
    if (mode === 'focused') {
      return <aura-focused style={{ width: '100%', height: '100%' }} />
    } else {
      return (
        <div id="aura-full-body" style={{ width: '100%', maxWidth: 320, marginTop: 20 }}>
          <aura-crown />
          <aura-top />
          <aura-bottom />
          <aura-base-inner />
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'url(/orchestra-bg.jpg) center/cover no-repeat', color: '#fff' }}>
      ...
      <div style={{ flex: 1.1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 20, borderLeft: '1px solid #333', borderRight: '1px solid #333', overflowY: 'auto', height: '100vh' }}>
        ...
        />
        <h1>AURA</h1>
        <button onClick={() => setMode(mode === 'focused' ? 'full-body' : 'focused')} style={{ marginBottom: 20, padding: 8, background: '#0af', color: '#000', borderRadius: 6 }}>Toggle Mode</button>
        {renderAvatar()}
