...
export default function ArtBusinessPage() {
  ...
  const handleStepClick = (field: string) => {
    if (field === "account_created") {
      ...
      setCustomModalContent(
        <div>
          <h3>Business Info</h3>
          <input placeholder="Business Name" onChange={e => setFieldValue(e.target.value)} style={{ marginBottom: 10, padding: 8, width: "100%" }} />
      ...
      setCustomModalContent(
        <div>
          ...
          <p>Integrate with WalletConnect or Stripe</p>
          <button onClick={() => alert("Wallet or Stripe connected")}>Connect</button>
        </div>
      );
      setShowCustomModal(true);
  ...
  const handleSaveField = async () => {
    ...
    await supabase.from("onboarding_audit").insert({ user_id: user.id, field_name: currentField, new_value: fieldValue, flow_type: userFlow });
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#fff", position: "relative", background: "#000" }}>
      ...
      <p style={{ textAlign: "center", fontSize: 18 }}>Progress: {Math.floor((currentFields.filter(f => userData[f]).length / currentFields.length) * 100)}%</p>
