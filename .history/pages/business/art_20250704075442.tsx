...
// Save-card form using Stripe SetupIntent
  ...
  return (
    <form onSubmit={handleSubmit} style={{ padding: 16 }}>
      <Elements stripe={stripePromise}>
        ...
        <button type="submit" disabled={!stripe} style={{ marginTop: 12, padding: '8px 12px', borderRadius: 4 }}>
          Save Card
        </button>
...
export default function BusinessArtPage() {
  ...
  const loadUserData = async () => {
    ...
    const u = auth?.user; if (!u) { setUser(null); return }
    setUser(u)
            ...
            {currentField === 'display_image' ? (
              ...
              fieldValue === 'card' ? (
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => { setFieldValue('paypal'); handleSaveField() }}>PayPal</button>
                  <button onClick={() => setFieldValue('card')}>Save Card</button>
                </div>
              )
            ) : currentField === 'art_role' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input value={bizName} onChange={e => setBizName(e.target.value)} placeholder="Business Name" />
                <input value={bizRole} onChange={e => setBizRole(e.target.value)} placeholder="Your Role" />
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {portfolioOptions.map(opt => (
                  <label key={opt} style={{
                    ...
                    background: (fieldValue || userData.portfolio || []).includes(opt) ? '#39ff14' : '#444',
                    color: '#000'
                  }}>
                    <input
                      type="checkbox"
                      checked={(fieldValue || userData.portfolio || []).includes(opt)}
                      onChange={() => {
                        const arr = fieldValue || userData.portfolio || []
