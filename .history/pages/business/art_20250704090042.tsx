...
export default function BusinessArtPage() {
  ...
  const handleWalletConnect = async (method:string) => {
    if (!user) return
    if (method==='cashapp') {
      // simulate Cash App connect
      const update={ user_id:user.id, wallet_connected:'Cash App' }
      await supabase.from('business_profiles').upsert(update,{onConflict:'user_id'})
      setUserData(prev=>({ ...prev, wallet_connected:'Cash App' })); setSaveMessage('✅ Connected Cash App')
  ...
  return (
    <>
      ...
      {/* Wallet Section with Big Connect Button */}
      <section style={{padding:24,background:'#222',textAlign:'center'}}>
        {!userData.wallet_connected ? (
          <button onClick={() => handleWalletConnect('cashapp')}
            style={{padding:'16px 24px',fontSize:18,fontWeight:'bold',background:'#39ff14',color:'#000',border:'none',borderRadius:8,cursor:'pointer'}}
          >
            Get Started Accept Payments
          </button>
        ) : (
          <WalletPanel />
        )}
