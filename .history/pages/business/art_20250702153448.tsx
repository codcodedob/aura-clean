...
export default function ArtBusinessPage() {
  ...
  const currentField = editingFieldIndex !== null ? arcFields[editingFieldIndex] : null;
  ...
  const handleSubmitField = async () => {
    ...
    const field = arcFields[editingFieldIndex];
    ...
    if (!error) {
      ...
      if (nextIndex < arcFields.length) {
        setEditingFieldIndex(nextIndex);
        setFieldValue(updated[arcFields[nextIndex]] || "");
      } else {
