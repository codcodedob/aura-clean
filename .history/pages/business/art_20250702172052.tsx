...
export default function ArtBusinessPage() {
  ...
  const handleGenerateHalo = async () => {
    ...
    if (!userError) {
      ...
      console.error("Halo generation failed:", userError);
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#fff", position: "relative" }}>
      ...
      <p style={{ textAlign: "center", fontSize: 18 }}>Progress: {Math.floor((arcFields.filter(f => userData[f]).length / arcFields.length) * 100)}%</p>
      {!haloExists && (
        <div style={{ textAlign: "center", marginTop: 20 }}>
          <button onClick={handleGenerateHalo} style={{ background: "#39ff14", color: "black", padding: "10px 20px", fontWeight: 700, borderRadius: 8 }}>Generate Halo Arc</button>
        </div>
      )}
