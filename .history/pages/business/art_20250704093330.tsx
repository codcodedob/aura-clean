...
export default function BusinessArtPage() {
  ...
  return (
    <>
      ...
      <div style={{
        position: 'sticky', top: 0, background: '#111', padding: 16,
        ...
        <div>
          ...
          <button
            ...
            style={{
              color: userFlow === 'business' ? '#39ff14' : '#ccc',
              background: 'none', border: 'none', fontWeight: 600
            }}
          >
            Business
      ...
      <div style={{
        ...
        padding: 16, background: '#000'
      }}>
        {fields.map((f, i) => (
          <motion.button
            key={f}
            onClick={() => {
              if (f === 'account_created' && !user) {
        ...
        {editingFieldIndex !== null && (
          <motion.div
            ...
            style={{
              ...
              background: '#111', padding: 24, borderRadius: 8, zIndex: 100,
              width: '90%', maxWidth: 400
            }}
          >
            <h3 style={{ marginBottom: 12 }}>Edit {currentField}</h3>
            {/* TODO: implement field-specific inputs */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button
                ...
                style={{ background: '#39ff14', color: '#000', padding: '6px 12px', borderRadius: 4 }}
              >
                Save
              </button>
            </div>
      ...
      <div style={{ width: '100vw', height: '20vh', background: '#111' }}>
        ...
        >
          <div style={{ position: 'absolute', transform: 'translate(-50%, -50%)' }}>
            <span style={{ background: '#39ff14', color: '#191c24', borderRadius: '50%', padding: 8, fontWeight: 700 }}>
              ◎
            </span>
          </div>
        </GoogleMapReact>
      ...
      <section style={{ padding: 24, background: '#111' }}>
        ...
        <div style={{ display: 'flex', overflowX: 'auto', gap: 12 }}>
          <div style={{ background: '#222', padding: 16, borderRadius: 8, minWidth: 200 }}>
            <h3 style={{ color: '#39ff14', margin: 0 }}>Plan Your Project</h3>
            <button onClick={() => router.push('/idea/create')} style={{ marginTop: 10 }}>Create</button>
          </div>
          <div style={{ background: '#222', padding: 16, borderRadius: 8, minWidth: 200 }}>
            <h3 style={{ color: '#39ff14', margin: 0 }}>Go to Space</h3>
      ...
      <section style={{ padding: 24, background: '#222' }}>
        ...
        <div style={{ marginTop: 16 }}>
          <button
            onClick={() => ready && open()}
            disabled={!ready}
            style={{ background: '#39ff14', color: '#000', padding: '12px 24px', border: 'none', borderRadius: 6, cursor: ready ? 'pointer' : 'not-allowed' }}
          >
      ...
      <section style={{ padding: 24, background: '#000' }}>
        ...
        <h2 style={{ color: '#39ff14', marginTop: 24, marginBottom: 12 }}>Live Tickets</h2>
        <div style={{ display: 'flex', overflowX: 'auto', gap: 12 }}>
          {liveTickets.map(t => (
            <div
              key={t.id}
            ...
            >
              {showQR[t.id] ? (
                <QRCode value={t.id} size={80} fgColor='#39ff14' bgColor='#000' />
              ) : (
                <>
