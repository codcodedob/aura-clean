...
export default function ArtBusinessPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#fff", position: "relative", overflow: "hidden" }}>
      <video src={videoUrl || DEFAULT_VIDEO} autoPlay loop muted playsInline style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", objectFit: "cover", zIndex: 0, opacity: 0.16, pointerEvents: "none", background: "#000" }} />
      ...
      <div style={{ position: 'sticky', top: 0, background: '#111', zIndex: 5, borderBottom: '1px solid #222' }}>
        <div style={{ padding: '0.5rem 1rem', fontSize: '0.9rem', color: '#39ff14', fontWeight: 700 }}>
          ...
          <button onClick={() => setUserFlow(userFlow === "arc" ? "business" : "arc")} style={{ marginLeft: '1rem', fontSize: '0.8rem', background: '#222', color: '#0af', padding: '0.25rem 0.5rem', border: 'none', borderRadius: 4 }}>Switch to {userFlow === "arc" ? "Business" : "ArcSession"}</button>
          {!haloExists && <button onClick={handleGenerateHalo} style={{ marginLeft: '1rem', fontSize: '0.8rem', background: '#39ff14', color: '#000', padding: '0.25rem 0.75rem', border: 'none', borderRadius: 4, fontWeight: 600 }}>Generate Halo Arc</button>}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', padding: '1rem', gap: '1.5rem' }}>
          {currentFields.map((field, index) => (
            <motion.button key={field} onClick={() => { setEditingFieldIndex(index); setFieldValue(userData[field] || ""); }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} style={{ background: 'transparent', color: userData?.[field] ? '#0af' : '#ff4d4d', border: 'none', fontSize: '1rem', fontWeight: 700, borderBottom: '2px solid #444', padding: '0.5rem 1rem' }}>
              {userData?.[field] ? `✓ ${field}` : `${field} (❗)`}
            </motion.button>
          ))}
          <motion.button onClick={() => router.push("/artgang")} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ marginLeft: '1rem', background: '#222', color: '#39ff14', border: 'none', padding: '0.5rem 1rem', borderRadius: 8, fontWeight: 700, fontSize: '1rem', boxShadow: '0 1px 8px #39ff1450' }}>
            artgang→
          </motion.button>
        </div>
        ...
        {editingFieldIndex !== null && (
          <div style={{ padding: '1rem', background: '#191919', textAlign: 'center' }}>
            ...
            <button onClick={handleSubmitField} style={{ padding: '0.5rem 1rem', background: '#39ff14', color: '#000', border: 'none', borderRadius: 6, fontWeight: 700 }}>Save</button>
            {saveMessage && <div style={{ marginTop: 10, fontSize: '0.9rem', color: saveMessage.includes("✅") || saveMessage.includes("🛸") ? '#39ff14' : '#ff4d4d' }}>{saveMessage}</div>}
          </div>
        )}
      </div>
