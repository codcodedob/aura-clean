...
export default function BusinessArtPage() {
  ...
  return (
    <React.Fragment>
      ...
      {/* Map */}
      <div style={{ height:'20vh', width:'100%', background:'#111' }}>
        <GoogleMapReact
      ...
      <section style={{ padding:24, background:'#000' }}>
        ...
        <div style={{ display:'flex', overflowX:'auto', gap:12 }}>
          {(famAwards.length?famAwards:[{id:'demo',title:'Demo',img_url:'/awards/artist1.jpg',winner:'Demo'}]).map(a=>(
      ...
      {/* IdeaFlight & Wallet */}
      <section style={{ display:'flex', gap:24, padding:24, background:'#111' }}>
        <div style={{ flex:1, background:'#222', padding:16, borderRadius:8 }}>
          <h3 style={{ color:'#39ff14' }}>IdeaFlight</h3>
          <p style={{ color:'#ccc', fontSize:14 }}>Plan and launch your creative projects.</p>
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={()=>router.push('/idea/create')} style={{ flex:1 }}>Create</button>
            <button onClick={()=>router.push('/space')} style={{ flex:1 }}>Go to Space</button>
          </div>
        </div>
        <div style={{ flex:1, background:'#222', padding:16, borderRadius:8 }}>
          <WalletPanel />
      ...
      {saveMessage && <div style={{ position:'fixed', bottom:16, left:'50%', transform:'translateX(-50%)', background:'#000', color:'#39ff14', padding:'8px 16px', borderRadius:4 }}>{saveMessage}</div>}
    </React.Fragment>
  )
}
