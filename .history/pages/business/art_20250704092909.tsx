...
export default function BusinessArtPage() {
  ...
  return (
    <>        
      ...
      <div style={{ position: 'sticky', top: 0, background: '#111', padding: 16,
                    zIndex: 10, display: 'flex', justifyContent: 'space-between',
                    alignItems: 'center' }}>
        <div>
          <button onClick={() => setUserFlow('arc')} style={{ marginRight: 12,
            color: userFlow === 'arc' ? '#39ff14' : '#ccc', background: 'none',
            border: 'none', fontWeight: 600 }}>
            ArcSession
          </button>
          <button onClick={() => setUserFlow('business')} style={{
            color: userFlow === 'business' ? '#39ff14' : '#ccc', background: 'none',
            border: 'none', fontWeight: 600 }}>
            Business
          </button>
        </div>
      ...
      <AnimatePresence>
        {editingFieldIndex !== null && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            ...
            {/* field-specific inputs go here */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => setEditingFieldIndex(null)}
                      style={{ background: 'transparent', color: '#ccc' }}>Cancel</button>
      ...
      <section style={{ padding: 24, background: '#111' }}>
        <h2 style={{ color: '#39ff14', marginBottom: 12 }}>IdeaFlight</n
