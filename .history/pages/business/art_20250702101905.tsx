// Final onboarding UI with asteroid-style magnetic line, animated debris, artgang button, and background video
...
function DebrisParticle({ position, velocity }) {
  const ref = useRef()
  useFrame(() => {
    if (ref.current) {
      ...
      if (ref.current.position.length() > 10) {
  ...
  return (
    <mesh ref={ref} position={position}>
      ...
      <meshStandardMaterial color="#fecf2f" emissive="#21f373" />
    </mesh>
  )
}
...
function DebrisField() {
  const particles = Array.from({ length: 40 }, (_, i) => {
    const pos = [Math.random() * 6 - 3, Math.random() * 3 - 1.5, Math.random() * 2 - 1]
    const vel = [0.001 - Math.random() * 0.002, 0.001 - Math.random() * 0.002, 0.001 - Math.random() * 0.002]
    return <DebrisParticle key={i} position={pos} velocity={vel} />
  })
  return <>{particles}</>
}
...
export default function ArtDepartment() {
  ...
  return (
    <div style={{ padding: 0, background: 'transparent', minHeight: '100vh', color: '#fff', position: 'relative' }}>
      {/* Background Video */}
      <video
        autoPlay
        muted
