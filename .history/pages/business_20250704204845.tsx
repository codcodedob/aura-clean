...
const ADMIN_EMAIL = "burks.donte@gmail";
...
export default function Business() {
  ...
  // --- TypeScript-safe change handler for both input and textarea ---
  ...
  // Handles onboarding/payment, ensures coin is present
  const handleSubmitModal = async (e: React.FormEvent) => {
    ...
    try {
      if (modalInfo?.key === "artist-coin") {
        // Create coin for this user (inactive)
        const { data: coin, error } = await supabase
          .from("aura_coins")
          .insert({
            name: formState.coinName,
            scope: [formState.coinScope],
            dividend_eligible: !!formState.coinDividends, // typo fixed from `dividends_eligible`
        ...
        // Paid product: get/create user coin first!
