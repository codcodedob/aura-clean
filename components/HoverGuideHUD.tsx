...
const HoverGuideHUD: React.FC<HoverGuideHUDProps> = ({ isSignedIn, pulse = false }) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (!dragging) return;
      setPos((prev) => {
        const newX = Math.min(Math.max(0, prev.x + e.movementX), window.innerWidth - 90);
        const newY = Math.min(Math.max(0, prev.y + e.movementY), window.innerHeight - 90);
  ...
  return (
    <>
      ...
      >
        ...
        {/* CSS Spinning coin (optional, only if not signed in) */}
        {!isSignedIn && (
          <span
            style={{
              position: "absolute",
