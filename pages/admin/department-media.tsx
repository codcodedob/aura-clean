...
export default function AdminDepartmentMedia() {
  ...
  function RenderUploader({ id, field, current, accept }: { id: string; field: "img_url" | "video_url"; current: string | null; accept: string }) {
    ...
    return (
      <div style={{ marginBottom: 8 }}>
        ...
        {field === "img_url" ? (
          current && <img src={current} style={{ width: 70, height: 44, objectFit: "cover", borderRadius: 6, border: "1px solid #ccc" }} alt="img" />
        ) : (
          current && <video src={current} controls style={{ width: 80, height: 44, objectFit: "cover", borderRadius: 6 }} />
        )}
      </div>
    );
  }
              ...
              return (
                <div key={slot} style={{ marginBottom: 36, padding: 16, borderRadius: 16, border: "1px solid #ccc", background: "#f8fafd" }}>
                  <div style={{ display: "flex", alignItems: "center", marginBottom: 9 }}>
                    ...
                    {entry.id !== "new" && entry.updated_at && (
                      <span style={{ fontSize: 13, color: "#bbb", marginLeft: 8 }}>{entry.updated_at.slice(0, 10)}</span>
                    )}
                  </div>
                  <div style={{ marginBottom: 7 }}>
                    <input
                      ...
                      onChange={e => {
                        ...
                        setMedia(m => m.map(n => n.id === entry.id ? { ...n, title: val } : n));
                        setEdit(ed => ({ ...ed, [entry.id + '-title']: true }));
                      }}
                      onBlur={() => entry.id !== "new" && edit[entry.id + '-title'] && updateField(entry.id, 'title', entry.title)}
                      placeholder="Title"
                  ...
                  <div style={{ marginBottom: 7 }}>
                    <textarea
                      ...
                      onChange={e => {
                        const val = e.target.value;
                        setMedia(m => m.map(n => n.id === entry.id ? { ...n, description: val } : n));
                        setEdit(ed => ({ ...ed, [entry.id + '-description']: true }));
                      }}
                      onBlur={() => entry.id !== "new" && edit[entry.id + '-description'] && updateField(entry.id, 'description', entry.description)}
                  ...
                  <div style={{ marginBottom: 7 }}>
                    <input
                      ...
                      onChange={e => {
                        const val = e.target.value;
                        setMedia(m => m.map(n => n.id === entry.id ? { ...n, link_url: val } : n));
                        setEdit(ed => ({ ...ed, [entry.id + '-link_url']: true }));
                      }}
                      onBlur={() => entry.id !== "new" && edit[entry.id + '-link_url'] && updateField(entry.id, 'link_url', entry.link_url)}
